  ...
  if (mail) {
    try {
      ...
      const htmlBody = dbTpl?.html || emailTemplate({
        ...
        body: `You've been invited to join <strong style="color:#ffffff">${mail.appName}</strong>. Click below to create your account. This invite expires in 3 days.`,
        buttonText: "Accept Invitation",
        buttonUrl: invite_url,
        footer: `Invited by an administrator · Expires ${new Date(expiresAt).toLocaleDateString()}`,
      });
