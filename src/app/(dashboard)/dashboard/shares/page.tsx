...
export default function SharedLinksPage() {
  ...
  // New state for the selection popup
  ...
  const createLink = async () => {
    if (selectedIds.length === 0) {
      toastError("Please select at least one item to share");
      return;
    }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }} className="fade-in">
      ...
      <div className="card" style={{ padding: 0, overflow: "hidden" }}>
        {loading ? (
          ...
          <div style={{ padding: 48, textAlign: "center", color: "var(--muted)" }}>No shared links created yet.</div>
        ) : (
          links.map((l, i) => (
            <div key={l.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", borderBottom: i < links.length - 1 ? "1px solid var(--border-color)" : "none" }}>
              ...
              <div style={{ display: "flex", gap: 6 }}>
                ...
                <button className="btn-ghost" style={{ padding: "6px 12px", fontSize: 12 }} onClick={() => { navigator.clipboard.writeText(getFullUrl(l.token)); success("Link copied!"); }}>
                  📋 Copy
                </button>
                <button style={{ background: "none", border: "none", color: "#EF4444", cursor: "pointer", padding: 4, marginLeft: 4 }} onClick={() => deleteLink(l.id)}>
                  🗑️
                </button>
              </div>
            </div>
          ...
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", backdropFilter: "blur(5px)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }} onClick={() => setShowSelector(false)}>
            <div onClick={e => e.stopPropagation()} style={{ background: "var(--surface)", borderRadius: 16, width: "100%", maxWidth: 440, border: "1px solid var(--border-color)", display: "flex", flexDirection: "column", maxHeight: "85vh", boxShadow: "0 24px 60px rgba(0,0,0,0.5)" }}>
              ...
              <div style={{ padding: "20px 22px", borderBottom: "1px solid var(--border-color)" }}>
                <div style={{ fontWeight: 700, fontSize: 17 }}>Create Shared Link</div>
                <div style={{ fontSize: 13, color: "var(--muted)", marginTop: 4 }}>Select which subscriptions to include</div>
              </div>
              
              <div style={{ flex: 1, overflowY: "auto", padding: "8px 0" }}>
                {subs.filter(s => s.active).length === 0 ? (
                  <div style={{ padding: 30, textAlign: "center", color: "var(--muted)", fontSize: 14 }}>No active subscriptions to share</div>
                ) : (
              ...
              <div style={{ padding: "16px 22px", borderTop: "1px solid var(--border-color)", display: "flex", justifyContent: "flex-end", gap: 10 }}>
                ...
                <button className="btn-primary" onClick={createLink} disabled={selectedIds.length === 0}>
                  Create Link ({selectedIds.length})
                </button>
              </div>
            </div>
          </div>
