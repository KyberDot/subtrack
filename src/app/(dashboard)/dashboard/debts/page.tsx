  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }} className="fade-in">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
        {[["Total Owed", currencySymbol + fmt(totalOwed), "#EF4444"],["Total Debt", currencySymbol + fmt(totalDebt), "var(--muted)"],["Total Paid", currencySymbol + fmt(totalPaid), "#10B981"]].map(([l,v,c]) => (
              ...
              <div key={d.id} className="card" style={{ opacity: d.active && !done ? 1 : 0.65 }}>
                <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {d.due_date && <div style={{ fontSize: 12, color: "var(--muted)" }}>Due: {d.due_date}</div>}
                    <div style={{ marginTop: 10 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 5 }}>
                        <span style={{ color: "var(--muted)" }}>{currencySymbol}{fmt(convertToDisplay(d.paid, d.currency))} paid</span>
                        <span style={{ fontWeight: 700, color: done ? "#10B981" : "#EF4444" }}>{currencySymbol}{fmt(owed)} remaining</span>
                      </div>
                      <div style={{ height: 6, background: "var(--surface2)", borderRadius: 3, overflow: "hidden" }}>
                        <div style={{ width: `${pct}%`, height: "100%", background: done ? "#10B981" : d.color, borderRadius: 3, transition: "width 0.5s" }} />
                      </div>
                      <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 3 }}>{pct.toFixed(0)}% paid · {currencySymbol}{fmt(total)} total</div>
                    </div>
                  </div>
