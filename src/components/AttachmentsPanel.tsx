...
interface Props { subId?: number; debtId?: number; label?: string; }
...
export default function AttachmentsPanel({ subId, debtId, label = "Attachments" }: Props) {
  ...
  const upload = async (file: File) => {
    ...
    reader.onload = async e => {
      await fetch("/api/attachments", {
        ...
        body: JSON.stringify({ sub_id: subId, debt_id: debtId, name: file.name, mime_type: file.type, data: e.target?.result as string, size: file.size })
      });
      await load();
      setUploading(false);
    };
  ...
  return (
    <>
      ...
      {/* Attachment modal */}
      {open && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.65)", backdropFilter: "blur(4px)", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center", padding: 20, overflow: "hidden" }}
      ...
      {/* Viewer modal */}
      {viewing && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 400, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, overflow: "hidden" }}
